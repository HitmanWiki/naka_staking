...
/************************************
 * 🔐 Minimal ERC-20 ABI (balance, allowance, approve)
 ************************************/
const erc20Abi = [
  {
...
const stakingAbi = [
  ...
  // emergencyUnstake(uint256 _lockDuration)
  {
    inputs: [{ internalType: 'uint256', name: '_lockDuration', type: 'uint256' }],
  ...
  // stakedTokens(address, uint256) view returns (uint256 amount, uint256 timestamp)
  {
    inputs: [
...
const AppContent = () => {
  ...
  const stakingPools = [
    ...
    { id: '21-day', duration: '21 Days', apy: 200, lockDurationInSeconds: 21 * 24 * 60 * 60 }
  ];
  ...
  return (
    <div className="app-container" style={{ backgroundImage: `url('data:image/svg+xml;base64,${btoa(backgroundPattern)}')` }}>
      <style>{`
        ...
        .unstake-button { padding: .75rem 1.5rem; border-radius: .75rem; font-family: 'Bebas Neue', sans-serif; color: white; background-color: var(--danger-color); transition: transform .2s, background-color .2s; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.1), 0 2px 4px -1px rgba(0,0,0,0.06); }
        ...
        {showModal && (
          <div className="modal-overlay">
            <div className={`modal-container ${modalType === 'success' ? 'success' : 'error'}`}>
              ...
              <div>
                <h3 className="modal-title">{modalType === 'success' ? 'Meow-nificent!' : 'Oh no!'}</h3>
